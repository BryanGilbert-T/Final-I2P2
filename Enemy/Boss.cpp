...
std::pair<float,float> BossEnemy::getPlayerPos() const {
...
void BossEnemy::Draw(Camera cam) {
    Enemy::Draw(cam);
}

void BossEnemy::performPatrol(float dt) {
    // Move horizontally
    auto scene = dynamic_cast<PlayScene*>(
        Engine::GameEngine::GetInstance().GetScene("play")
    );
...
void BossEnemy::performChase(float dt, float dx, float dy, float dist) {
    // Normalize direction
    auto scene = dynamic_cast<PlayScene*>(
        Engine::GameEngine::GetInstance().GetScene("play")
    );
...
void BossEnemy::move(int keyCode) {
